...
# From here, modified a bit for MacOS tools
# https://github.com/CodeDefenders/CodeDefenders/blob/master/scripts/ci/generate-codequality-report-from-checkstyle.sh
...
tail -n +3 $1 | while read p; do
    ...
    if printf "$p" | grep -q "<error.*" -; then
        ...
        checksum=$(printf "%s %s %s\n" "$file" "$line" "$message" | shasum | awk '{print $1}')
        if [ "$first" == 1 ]; then
