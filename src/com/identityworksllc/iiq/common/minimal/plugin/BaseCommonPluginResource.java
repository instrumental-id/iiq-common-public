...
import com.identityworksllc.iiq.common.minimal.plugin.annotations.AuthorizeAll;
import com.identityworksllc.iiq.common.minimal.plugin.annotations.AuthorizeAny;
import com.identityworksllc.iiq.common.minimal.plugin.annotations.AuthorizedBy;
import com.identityworksllc.iiq.common.minimal.plugin.annotations.NoReturnValue;
...
public abstract class BaseCommonPluginResource extends BasePluginResource implements CommonExtendedPluginContext {
		...
		try {
			...
			try {
				...
				try {
					...
					return handleResult(hasReturnValue, response);
				} catch(Exception e) {
	 ...
	 */
	private Response handleResult(boolean hasReturnValue, Object response) throws GeneralException {
		if (hasReturnValue) {
			if (response instanceof Response) {
				...
				return Response.ok().entity(response).build();
			} else if (isAllowedOutput(response)) {
				return Response.ok().entity(response).build();
